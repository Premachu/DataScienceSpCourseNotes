...
* **Subsetting data by multiple conditions** [tutorial] (https://stackoverflow.com/questions/25647470/filter-multiple-conditions-dplyr):
    * `dat$name %in% target` = for each value in dat$name, check that it exists in target.
    * `subset(df, v1 %in% condition)` 
    * `df[df$v1 %in% condition,]` 
    * `df[df$gift %in% wishlist,]` = subset Santa's gifts by the multiple items/conditions of the wishlist. 
...
* **re-order columns**
  * `df[,c(1,2,3,4)]` = original dataset. To re-order columns do
  * `df2 <- df[,c(1,3,2,4)]` = re-order columns. The first `,` means keep all rows, and the 1,2,3,4 refers to the columns
  * `df2 <- df[, c("v1","v3","v4")]` = re-order by column name
...
* **Finding values**
  * `grep("string", df$col)` = find intances which match value/string
      ...
      * `grep("cowboys", df, ignore.case = TRUE)` = case insensitive. Finds `Cowboy` and `cowboy`
...
* `.*something.*` = replace whole string / sentence
    * *example*: `x <- c("strong wind, weak wind")` = create string  
    `gsub("wind", "CHIPS", x)` = replaces only part of string, i.e. `strong CHIPS, weak CHIPS`  
    `gsub(".*wind.*", "CHIPS", x)` = replaces whole string, i.e. `CHIPS, CHIPS`  
    
## Working with dates

* `date()` = returns current date in character format
* `weeekdays()` = returns day of the week 
    * `Sys.setlocale("LC_TIME", "C")` = sets days in english, rather than portuguese 
...
* `as.Date("character", "format")` = converts character vector/variable into Date format by using the codes above
  ...
  * ***Note***: when formating strings into dates use the correct seperator
      * `as.Date("4/18/1950", "%m/%d/%Y")` = use forward slash sep
      * `as.Date("October 6, 2010", "%B %d, %Y")` = use comma sep after day
