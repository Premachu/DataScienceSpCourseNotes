...
* `sample(values, n, replace = FALSE)` = generate random samples
     ...
     * `lotto <- sample(1:40, 6, replace=F)` = randomly select 6 balls (each labelled from 1 to 40), without repeats (the ball only appears once). 
        * ***note***: `replace=F` is the default, but added for clarity
    ...
    * `sample(pokemon.name, 10)` = select 10 items from a list of 151
    * `sample(vector)` = can be used to randomise the order of a vector. i.e. all pokemon in random order
